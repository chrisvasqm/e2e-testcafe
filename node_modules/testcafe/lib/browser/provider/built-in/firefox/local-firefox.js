            ...
            while (1) {
                switch (_context.prev = _context.next) {
                    ...
                    case 3:
                        ...
                        _context.next = 9;
                        return _testcafeBrowserTools2.default.open(firefoxOpenParameters, pageUrl);
...
function buildFirefoxArgs(config, platformArgs, profileDir) {
    return [].concat(!config.userProfile ? ['-no-remote', '-new-instance', '-profile "' + profileDir.name + '"'] : [], config.headless ? ['-marionette', '-headless'] : [], config.userArgs ? [config.userArgs] : [], platformArgs ? [platformArgs] : []).join(' ');
}