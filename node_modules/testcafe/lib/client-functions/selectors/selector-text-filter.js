...
function selectorTextFilter(node, index, originNode, textRe) {
    function hasChildrenWithText(parentNode) {
        var cnCount = parentNode.childNodes.length;

        for (var i = 0; i < cnCount; i++) {
            if (selectorTextFilter(parentNode.childNodes[i], index, originNode, textRe)) return true;
        }
    ...
    // DocumentFragment
    if (node.nodeType === 11) return hasChildrenWithText(node, textRe);
